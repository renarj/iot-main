...
import com.oberasoftware.iot.core.robotics.commands.ReadAngleLimit;
...
public class DynamixelServoDataManager implements ServoDataManager, EventHandler {
    ...
    public <T> T readServoProperty(String servoId, ServoProperty property) {
        ...
        if(holder.isUpdatedInTimeFrame(property, 1000, TimeUnit.MILLISECONDS)) {
            ...
            if (property == ServoProperty.POSITION || property == ServoProperty.SPEED) {
                ...
                eventBus.publish(new ReadAngleLimit(servoId));
            } else if (property == ServoProperty.TORGUE) {
                eventBus.publish(new ReadTorgueCommand(servoId));
            }
