 ...
 */
@Component
public class IfXMLBlockParser implements XMLBlockParser<IfStatement> {
    ...
    @Override
    public IfStatement parse(Element node) throws BlocklyParseException {
    ...
    private IfBranch getBranch(Element node, String ifBranchName, String statementName) throws BlocklyParseException {
        ...
        List<Statement> statements = getStatements(statementBlock);
        ...
        if(ifCondition.isPresent()) {
            ...
            XMLBlockParser<Condition> conditionBlockParser = blockParserFactory.getParser(conditionBlock.getAttribute("type"));
    ...
    private List<Statement> getStatements(Element statementBlock) throws BlocklyParseException {
        XMLBlockParser<Statement> actionBlockParser =
                blockParserFactory.getParser(statementBlock.getAttribute("type"));
        Statement statement = actionBlockParser.parse(statementBlock);
        List<Statement> statements = Lists.newArrayList(statement);
