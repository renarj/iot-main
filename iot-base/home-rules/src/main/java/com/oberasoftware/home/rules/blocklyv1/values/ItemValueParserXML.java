 ...
 */
@Component
public class ItemValueParserXML implements XMLBlockParser<ResolvableValue> {
    private static final Logger LOG = getLogger(ItemValueParserXML.class);
    ...
    public ResolvableValue parse(Element node) throws BlocklyParseException {
        ...
        return new ThingAttributeValue(controllerId, thingId, label);
    }

    public static String getLabelValue(ResolvableValue resolvableValue) throws BlocklyParseException {
        if(resolvableValue instanceof StaticValue staticValue) {
