...
bool dxl_debug_menu_shwo_ctrltbl()
{
  ...
  addr = (uint32_t)&p_dxl_mem->LIDAR_DIST - (uint32_t)p_dxl_mem;
  DEBUG_SERIAL.print(addr);
  DEBUG_SERIAL.print("\t LIDAR     \t "); DEBUG_SERIAL.print  (p_dxl_mem->LIDAR_DIST);
  DEBUG_SERIAL.print("\t 0x");                  DEBUG_SERIAL.println(p_dxl_mem->LIDAR_DIST,HEX);
