...
static uint8_t lidarReqCount = 0;
static uint32_t lidarDist = 0;
...
void lidarUpdate() {
  bool bias = lidarReqCount == 0 ? true : false;
  
  lidarDist = myLidarLite.distance(bias); 
  
  lidarReqCount = lidarReqCount + 1;

  if(lidarReqCount > 100) {
    lidarReqCount = 0;
  }
}
